...
func (m *Meta) backend_c_r_S(c *configs.Backend, sMgr state.State, output bool) (backend.Backend, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	s := sMgr.State()
	...
	if err != nil {
		diags = diags.Append(fmt.Errorf(strings.TrimSpace(errBackendLocalRead), err))
		return nil, diags
	}
	...
	if err != nil {
		diags = diags.Append(fmt.Errorf(strings.TrimSpace(errBackendSavedUnsetConfig), s.Backend.Type, err))
		return nil, diags
	}
...
func (m *Meta) backend_C_r_S_unchanged(c *configs.Backend, sMgr state.State) (backend.Backend, tfdiags.Diagnostics) {
	s := sMgr.State()
